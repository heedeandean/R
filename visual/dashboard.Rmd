...
#install.packages('flexdashboard') # R Markdown 기반
...
kor_ex_xts <- getSymbols(Symbols = 'KRW=X', 
                          ...
                          auto.assign = FALSE)[,4]
names(kor_ex_xts) <- '원/달러 환율'

yen_xts <-  getSymbols(Symbols = 'JPY=X', 
                       from='2010-01-01', to=Sys.Date(),
                       auto.assign = FALSE)[,4]
names(yen_xts) <- '엔/달러 환율'

euro_xts <-  getSymbols(Symbols = 'EUR=X', 
                       from='2010-01-01', to=Sys.Date(),
                       auto.assign = FALSE)[,4]
names(euro_xts) <- '유로/달러 환율'
...
valueBox(ex1, icon = 'fa-dollar-sign', caption = paste('원: ', index(tail(kor_ex_xts,1))))
```
### 엔/달러 환율
```{r}
ex2 <- as.integer(tail(yen_xts,1))
valueBox(ex2, icon = 'fa-yen-sign', color='red', caption = paste('엔 환율: ', index(tail(yen_xts,1))))
```

### 유로/달러 환율
```{r}
ex3 <- as.integer(tail(euro_xts,1)*100)/100
valueBox(ex3, icon = 'fa-euro-sign', color='green', caption = paste('유로: ', index(tail(euro_xts,1))))
```
Column {data-width=650}
-----------------------------------------------------------------------
### 원/달러 환율
```{r}
aa1 <- dygraph(kor_ex_xts) # 그래프에 마우스를 대면 값을 볼 수 있음.
dyRangeSelector(aa1)
```
...
valueBox(ss2, icon = 'fa-chart-bar', color='red', caption = paste('S&P 500: ', index(tail(kospi_xts,1))))
```
### FTSE
```{r}
